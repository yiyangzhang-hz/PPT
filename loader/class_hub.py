...
class ClassHub:
    ...
    def get_class_list(self):
        ...
        file_paths = glob.glob(f'{self.module_dir}/*_{self.module_type}.py') #匹配所有格式为*_operator.py的文件
        class_list = []
        for file_path in file_paths:
            file_name = file_path.split('/')[-1].split('\\')[-1].split('.')[0]
            module = importlib.import_module(f'{self.module_dir.replace("/", ".")}.{file_name}') #导入模块
