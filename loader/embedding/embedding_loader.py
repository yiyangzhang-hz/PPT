...
class EmbeddingLoader:
    ...
    def load(self):
        if hasattr(self, 'get_{}_embedding'.format(self.type)): #{}的内容由self.type决定，如果self中包含这个get_{}_embedding的属性或者方法则为True
            getter = getattr(self, 'get_{}_embedding'.format(self.type)) #来调用相关的方法
