...
class Worker:
    def __init__(self, config):
        ...
        # Controller是来自/loader/controller.py 的一个class
        ...
        self.legommender = self.controller.legommender.to(Meta.device) #包括了News encoder, User encoder等最重要的部件
        self.resampler = self.controller.resampler #包含了item_cache，这是经由natural concator之后的完全由tokens所组成的序列
    ...
    def train(self) -> int:
        ...
        loader = self.controller.get_loader(Phases.train).train() #DataLoader，其中cacher的item存储了BERT第layer+1层的一个输出结果
        ...
        for epoch in range(self.exp.policy.epoch_start, self.exp.policy.epoch + self.exp.policy.epoch_start): #从0到50
            ...
            self.legommender.train() #设置为训练模式，这里最终调用的实际上是torch.nn.Moudle中的train()函数
            loader.train() #也是一堆训练模式的设置
            # 一个batch中包含了history(nid形式), candidates(key为nid，1+K个), click(batch_size个), imp(batch_size个), uid((batch_size个))，已经buffle过，都是ID形式
            for step, batch in enumerate(tqdm(loader, disable=self.disable_tqdm)):  #不同epoch下的每个step的batch是不同的，但是同个epoch在每次运行下的batch都是一样的，这是为了reproducibility
                # if step >= 1000:
                #     break
    ...
    def test(self):
        loader = self.controller.get_loader(Phases.test).test() #加载数据集，包括已经cache过的user和item的embedding
    ...
    def base_evaluate(self, loader, cols):
        score_series = torch.zeros(len(loader.dataset), dtype=torch.float32) #len(loader.dataset)是测试数据中所有形如N1234-0的impression的个数
        col_series = {col: torch.zeros(len(loader.dataset), dtype=torch.long) for col in cols} #包含两个字典，key分别是click和imp，然后初始化为一个len(loader.dataset)的全0的tensor

        index = 0
        for step, batch in enumerate(tqdm(loader, disable=self.disable_tqdm)): #batch包含nid, click, imp, uid，其中click标识是否点击，imp是每条behavior数据的index，一条数据对应同一个imp
            with torch.no_grad():
                scores = self.legommender(batch=batch) #计算一个batch中，每个user-news配对的分数
    ...
    def evaluate(self, loader, metrics):
        ...
        label_col, group_col = self.controller.column_map.label_col, self.controller.column_map.group_col #label_col是表示是否点击的01，group_col是impression
        score_series, col_series = self.base_evaluate(loader, cols=[label_col, group_col])
        label_series, group_series = col_series[label_col], col_series[group_col] #label_series记录每个user-news对得到点击情况（0 or 1），group_series记录每个user-news所属的分组，即同一条behavior数据是同一个分组
    ...
    对于BERT而言，pretrained_parameters至少包含6个，分别是position_embeddings，token_type_embeddings，LayerNorm所需要的weight(缩放)和bias(位移)，以及在所有层结束后的pooler的weight和bias
    ...
    def train_runner(self):
        if self.legommender.config.use_item_content and self.exp.policy.item_lr:
            ...
            pretrained_parameters, other_parameters = self.legommender.get_parameters() #获取需要训练的参数
    ...
    def test_llm_layer_split(self):
        ...
        pager = LLMSplitPager(
            ...
            contents=self.resampler.item_cache,  # resample.item_cache中存储了item的natural concator的结果，用token序列表示
...
if __name__ == '__main__':
    configuration = ConfigInit(
        ...
        default_args=dict(
            ...
            page_size=1024, #每次cache的数据条数，调的越小占的内存越小，其他应该没区别
            patience=5,
            epoch_start=0,
            frozen=True,
            load_path=None,
