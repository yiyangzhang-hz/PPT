...
class BaseOperator(BaseModule):
    ...
    def prepare_for_predictor(self, user_embeddings, candidate_size):
        ...
        user_embeddings = user_embeddings.unsqueeze(1).repeat(1, candidate_size, 1)  # B, K+1, D, 相当于让每个user embedding都复制粘贴K份，这样后面可直接与candidate news embedding点积
        user_embeddings = user_embeddings.view(-1, user_embeddings.shape[-1]) #squeeze为(数据条数*(1+K))*(hidden_size)
