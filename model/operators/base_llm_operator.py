...
class BaseLLMOperatorConfig(AttentionOperatorConfig):
    def __init__(
            self,
            # llm_dir: str = '/home/zhangyy/code/LLM/bert-base-uncased',
            # llm_dir: str = '/home/zhangyy/code/LLM/yahma/llama-7b-hf',
            # llm_dir: str = '/private/task/zhangyy/code/LLM/bert-base-uncased',
            llm_dir: str = '/private/task/zhangyy/code/LLM/yahma/llama-7b-hf',
...
class BaseLLMOperator(BaseOperator):
    ...
    def layer_split(self, num_hidden_layers):
        if self.config.layer_split: #0的话为False
    ...
    def forward(self, embeddings, mask=None, **kwargs):
        if not self.config.layer_split:
            ...
            if self.ifeng:
                ...
                hidden_states = self.hidden_weights_eng[indices]  # [B, L, D] #相当于是所有的candidates的第layer+1层输出的hidden_states
                mask = self.attention_mask_eng[indices]  # [B, L, L]
                ...
                indices = embeddings  # [B]
                hidden_states = self.hidden_weights[indices]  # [B, L, D] #相当于是所有的candidates的第layer+1层输出的hidden_states
